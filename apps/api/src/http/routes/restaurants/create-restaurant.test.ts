...
describe("POST /restaurants", () => {
	...
	it("creates an restaurant without domain and returns 201", async () => {
		const userId = faker.string.uuid();
		const orgId = faker.string.uuid();

		prismaMock.restaurant.create.mockResolvedValue({ id: orgId });
		...
		expect(JSON.parse(response.body)).toMatchObject({ restaurantId: orgId });
	});

	it("creates an restaurant with an available domain", async () => {
		const userId = faker.string.uuid();
		const orgId = faker.string.uuid();
