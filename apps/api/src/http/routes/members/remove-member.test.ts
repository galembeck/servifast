...
describe("DELETE /restaurants/:slug/members/:memberId", () => {
	...
	it("removes the member when user is OWNER", async () => {
		...
		const response = await app.inject({
			method: "DELETE",
			url: `/restaurants/${restaurant.slug}/members/${memberId}`,
	...
	it("returns 400 NOT_FOUND when member does not exist in the restaurant", async () => {
		const userId = faker.string.uuid();
		const { restaurant } = mockMembership(userId, "OWNER");
		...
		const response = await app.inject({
			method: "DELETE",
			url: `/restaurants/${restaurant.slug}/members/${faker.string.uuid()}`,
			headers: { Authorization: `Bearer ${token}` },
		});
	...
	it("returns 401 INVALID_TOKEN when not authenticated", async () => {
		const response = await app.inject({
			method: "DELETE",
			url: `/restaurants/some-org/members/${faker.string.uuid()}`,
		});
